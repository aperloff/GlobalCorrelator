...
int main() {
	...
	ap_uint<12> int_inputs[6] = {0b000000000000, 0b000000000001, 0b111111111111, 0b111111111110, 0b100000000000, 0b011111111111};
	ap_fixed<12, 5, AP_RND_CONV, AP_SAT> fixed_inputs[6] = {0b000000000000, 0b000000000001, 0b111111111111, 0b111111111110, 0b100000000000, 0b011111111111};
	
	for (unsigned int input = 0; input < n_inputs; ++input) {
		std::cout << "\nRaw Input (ap_uint Format)= " << int_inputs[input].to_string().c_str() << std::endl;
		std::cout << "Raw Input (ap_fixed Format)= " << fixed_inputs[input].to_string().c_str() << std::endl;
		ap_uint<12> inA = int_inputs[input];
		ap_fixed<12, 5, AP_RND_CONV, AP_SAT> inB = fixed_inputs[input];
